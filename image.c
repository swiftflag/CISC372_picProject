...
void* convolute(void* arg){
    ...
    int row,pix,bit;
    int chunkcalc = (args->srcImage->height/THREAD_COUNT);
    int start = args->threadid * chunkcalc;
    int end = start + chunkcalc;
    if (end > args->srcImage->height) end = args->srcImage->height;
    //int span = args->srcImage->bpp * args->srcImage->bpp;
...
int main(int argc,char** argv){
    ...
    for (int i = 0; i < THREAD_COUNT; i++) {
        ...
        if (pthread_create(&threads[i], NULL, convolute, &args[i]) != 0) {
